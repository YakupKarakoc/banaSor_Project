...
export default function AdminPanelScreen({ navigation, route }) {
  ...
  const handleDeleteForum = async (id) => {
    Alert.alert("Forum Sil", "Bu forumu silmek istiyor musunuz?", [
      { text: "İptal", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`${BASE_URL}/api/admin/forum/${id}`, {
              headers: { Authorization: `Bearer ${token}` },
            });
            setForums(prev => prev.filter(f => f.forumid !== id));
            Alert.alert("Başarılı", "Forum başarıyla silindi.");
          } catch (e) {
  ...
  const handleDeleteSoru = async (id) => {
    Alert.alert("Soru Sil", "Bu soruyu silmek istiyor musunuz?", [
      { text: "İptal", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`${BASE_URL}/api/admin/soru/${id}`, {
              headers: { Authorization: `Bearer ${token}` },
            });
  ...
  const handleDeleteGrup = async (id) => {
    Alert.alert("Grup Sil", "Bu grubu silmek istiyor musunuz?", [
      { text: "İptal", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`${BASE_URL}/api/admin/grup/${id}`, {
              headers: { Authorization: `Bearer ${token}` },
            });
